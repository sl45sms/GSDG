#!/bin/bash
#SBATCH -A a0140
#SBATCH --job-name=gsdg-curate-397b-async
...
# ============================================================================
# Async multi-node curation launcher for Qwen3.5-397B on Clariden.
#
# Replaces the sequential LLM-review loop of the older
# run_curate_qwen3_397b_clariden_multinode.sh with a concurrent async
# pipeline that keeps 16-32 review requests in flight, saturating vLLM's
# continuous-batching pipeline.
#
# Usage:
#
#   export INPUT_JSONL="${SCRATCH}/synthetic_chatml_397b.jsonl"
#   sbatch scripts/run_curate_qwen3_397b_clariden_async.sh
#
# Tunables:
#   CURATION_CONCURRENCY  – max in-flight review requests (default 16)
#   CURATION_BATCH_SIZE   – rows per progress-report batch (default 200)
#   DISABLE_LLM_REVIEW    – set to 1 to skip LLM review entirely
# ============================================================================
...
CURATION_OUT_DIR="${CURATION_OUT_DIR:-${SCRATCH}/synthetics}"
STATE_DB="${STATE_DB:-${CURATION_OUT_DIR}/.curation_state.sqlite3}"
...
# --- async curation knobs --------------------------------------------------
CURATION_CONCURRENCY="${CURATION_CONCURRENCY:-16}"
CURATION_BATCH_SIZE="${CURATION_BATCH_SIZE:-200}"
CURATION_LOG_LEVEL="${CURATION_LOG_LEVEL:-INFO}"
CURATION_TIMEOUT_SECONDS="${CURATION_TIMEOUT_SECONDS:-180}"
REVIEW_MAX_TOKENS="${REVIEW_MAX_TOKENS:-256}"
REVIEW_TEMPERATURE="${REVIEW_TEMPERATURE:-0.0}"
DISABLE_LLM_REVIEW="${DISABLE_LLM_REVIEW:-0}"

# --- validation ------------------------------------------------------------
if [[ -z "${INPUT_JSONL}" ]]; then
	echo "Set INPUT_JSONL to the generator-produced JSONL that should be curated." >&2
	exit 1
fi

if ! command -v readlink >/dev/null 2>&1; then
	echo "readlink is required to resolve INPUT_JSONL." >&2
	exit 1
fi

INPUT_JSONL="$(readlink -f "${INPUT_JSONL}")"
if [[ ! -f "${INPUT_JSONL}" ]]; then
	echo "INPUT_JSONL was not found: ${INPUT_JSONL}" >&2
	exit 1
fi

if [[ -z "${REJECT_LOG}" ]]; then
	input_stem="$(basename "${INPUT_JSONL}")"
	input_stem="${input_stem%.jsonl}"
	REJECT_LOG="${CURATION_OUT_DIR}/rejects_${input_stem}.jsonl"
fi
...
CURATE_ENTRYPOINT="/workspace/scripts/curate_jsonl_async.py"
...
if [[ "${STAGE_WORKSPACE}" != "0" ]]; then
	...
	CURATE_ENTRYPOINT="${STAGE_ROOT}/scripts/curate_jsonl_async.py"
...
# --- stage input JSONL into workspace if needed ----------------------------
if [[ -n "${SCRATCH:-}" && "${INPUT_JSONL}" == "${SCRATCH}/"* ]]; then
	CURATE_INPUT_PATH="${INPUT_JSONL}"
else
	mkdir -p "${STAGE_ROOT}/inputs"
	CURATE_INPUT_PATH="${STAGE_ROOT}/inputs/$(basename "${INPUT_JSONL}")"
	cp -f "${INPUT_JSONL}" "${CURATE_INPUT_PATH}"
	echo "Staged input into ${CURATE_INPUT_PATH}" >&2
fi

# --- export everything for the inner srun ----------------------------------
export CURATE_ENTRYPOINT RAY_SERVE_ENTRYPOINT SITECUSTOMIZE_INSTALLER CURATE_INPUT_PATH
export CURATION_OUT_DIR STATE_DB REJECT_LOG API_BASE MODEL_NAME TOKENIZER_MODEL
export CURATION_LOG_LEVEL CURATION_TIMEOUT_SECONDS REVIEW_MAX_TOKENS REVIEW_TEMPERATURE DISABLE_LLM_REVIEW
export CURATION_CONCURRENCY CURATION_BATCH_SIZE
export MAX_MODEL_LEN MASTER_PORT TENSOR_PARALLEL_SIZE PIPELINE_PARALLEL_SIZE
export REASONING_PARSER MASTER_ADDR RAY_PORT

echo "=== Async curation launcher ===" >&2
...
echo "Curating input snapshot: ${CURATE_INPUT_PATH}" >&2
echo "Curated outputs will be written to ${CURATION_OUT_DIR}" >&2
echo "Concurrency: ${CURATION_CONCURRENCY}  batch-size: ${CURATION_BATCH_SIZE}" >&2
if [[ "${DISABLE_LLM_REVIEW}" == "1" ]]; then
	echo "LLM review is DISABLED; only deterministic heuristics will be used." >&2
fi
...
SRUN_EXPORT+=",CURATE_ENTRYPOINT=${CURATE_ENTRYPOINT},SITECUSTOMIZE_INSTALLER=${SITECUSTOMIZE_INSTALLER}"
...
SRUN_EXPORT+=",CURATE_INPUT_PATH=${CURATE_INPUT_PATH},CURATION_OUT_DIR=${CURATION_OUT_DIR}"
SRUN_EXPORT+=",STATE_DB=${STATE_DB},REJECT_LOG=${REJECT_LOG},API_BASE=${API_BASE}"
SRUN_EXPORT+=",MODEL_NAME=${MODEL_NAME},TOKENIZER_MODEL=${TOKENIZER_MODEL}"
SRUN_EXPORT+=",CURATION_LOG_LEVEL=${CURATION_LOG_LEVEL},CURATION_TIMEOUT_SECONDS=${CURATION_TIMEOUT_SECONDS}"
SRUN_EXPORT+=",REVIEW_MAX_TOKENS=${REVIEW_MAX_TOKENS},REVIEW_TEMPERATURE=${REVIEW_TEMPERATURE}"
SRUN_EXPORT+=",DISABLE_LLM_REVIEW=${DISABLE_LLM_REVIEW}"
SRUN_EXPORT+=",CURATION_CONCURRENCY=${CURATION_CONCURRENCY},CURATION_BATCH_SIZE=${CURATION_BATCH_SIZE}"
...
# Bump file-descriptor limit for high concurrency connections.
...
# --- paths -----------------------------------------------------------------
SERVER_LOG="${PWD}/vllm-curate-397b-node${SLURM_NODEID}.log"
RAY_LOG="${PWD}/ray-curate-397b-node${SLURM_NODEID}.log"
...
# ============================================================================
# Node 0: Ray head + vLLM server + async curation
# ============================================================================
...
PY
	...
	# --- async curation ----------------------------------------------------
	curation_args=(
		/opt/gsdg-venv/bin/python "$CURATE_ENTRYPOINT"
		"$CURATE_INPUT_PATH"
		--out-dir "$CURATION_OUT_DIR"
		--state-db "$STATE_DB"
		...
		--skip-healthcheck
		--log-level "$CURATION_LOG_LEVEL"
		--curation-concurrency "$CURATION_CONCURRENCY"
		--curation-batch-size "$CURATION_BATCH_SIZE"
	)
	if [[ "$DISABLE_LLM_REVIEW" == "1" ]]; then
		curation_args+=(--disable-llm-review)
	fi

	"${curation_args[@]}"
	echo "Server log: $SERVER_LOG" >&2
	if [[ -f "$REJECT_LOG" ]]; then
		echo "Reject log: $REJECT_LOG" >&2
	fi
