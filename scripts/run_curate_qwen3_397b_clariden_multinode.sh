#!/bin/bash
#SBATCH -A a0140
#SBATCH --job-name=gsdg-curate-397b-clr
...
CURATE_ENTRYPOINT="/workspace/scripts/curate_jsonl.py"
...
if [[ "${STAGE_WORKSPACE}" != "0" ]]; then
	...
	CURATE_ENTRYPOINT="${STAGE_ROOT}/scripts/curate_jsonl.py"
...
SRUN_EXPORT+=",MASTER_ADDR=${MASTER_ADDR},MASTER_PORT=${MASTER_PORT},RAY_PORT=${RAY_PORT},CURATE_ENTRYPOINT=${CURATE_ENTRYPOINT},SITECUSTOMIZE_INSTALLER=${SITECUSTOMIZE_INSTALLER},VLLM_ENABLE_V1_MULTIPROCESSING=${VLLM_ENABLE_V1_MULTIPROCESSING},VLLM_ALLREDUCE_USE_SYMM_MEM=${VLLM_ALLREDUCE_USE_SYMM_MEM},NCCL_SOCKET_IFNAME=${NCCL_SOCKET_IFNAME},GLOO_SOCKET_IFNAME=${GLOO_SOCKET_IFNAME},NCCL_CROSS_NIC=${NCCL_CROSS_NIC},FI_PROVIDER=${FI_PROVIDER},FI_CXI_DEFAULT_CQ_SIZE=${FI_CXI_DEFAULT_CQ_SIZE},FI_CXI_DEFAULT_TX_SIZE=${FI_CXI_DEFAULT_TX_SIZE},FI_CXI_DISABLE_HOST_REGISTER=${FI_CXI_DISABLE_HOST_REGISTER},FI_CXI_RX_MATCH_MODE=${FI_CXI_RX_MATCH_MODE},FI_MR_CACHE_MONITOR=${FI_MR_CACHE_MONITOR},CURATE_INPUT_PATH=${CURATE_INPUT_PATH},CURATION_OUT_DIR=${CURATION_OUT_DIR},STATE_DB=${STATE_DB},REJECT_LOG=${REJECT_LOG},API_BASE=${API_BASE},MODEL_NAME=${MODEL_NAME},TOKENIZER_MODEL=${TOKENIZER_MODEL},CURATION_LOG_LEVEL=${CURATION_LOG_LEVEL},CURATION_TIMEOUT_SECONDS=${CURATION_TIMEOUT_SECONDS},REVIEW_MAX_TOKENS=${REVIEW_MAX_TOKENS},REVIEW_TEMPERATURE=${REVIEW_TEMPERATURE},DISABLE_LLM_REVIEW=${DISABLE_LLM_REVIEW}"
